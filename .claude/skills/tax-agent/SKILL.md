...
- **2025년 귀속분(Tax Year 2025): 신고 완료.** 다시 계산하거나 수정하지 않는다.
  25년 파일은 오직 출력 형식(형식 A)과 컬럼 구조의 참고용으로만 사용한다.
- **2026년 귀속분(Tax Year 2026): 현재 작업 대상.** 모든 기록, 정제, 출력은
  2026년 거래를 대상으로 한다.
  - 거래 장부: `tax-data/2026-trades.csv`
  - 결과 파일: `2026_Tax_Summary.xlsx`
- 한 줄 거래 입력에서 연도가 생략되면 **2026년**으로 기록한다.
  2026년이 아닌 거래일이 입력되면 기록 전에 한 줄로 확인을 요청한다.
- 일괄 정제([강제 워크플로우]) 시 입력 데이터에 2026년 외의 거래가 섞여 있으면:
  - 매도(Sale Date) 기준 2026년 거래만 신고 대상에 포함한다.
  - 2025년 이전 매수 행은 취득일/취득가 산출(원가 기초)에만 사용한다.
  - 신고 대상에서 제외한 행은 결과 하단에 "제외 내역"으로 별도 표기한다.
- 다음 해로 넘어가면(2027년 귀속분 작업 시) 장부와 결과 파일명을 해당 연도로
  새로 만들고, 이 섹션의 연도 표기를 갱신한다.
...
## 알려진 입력 파일 형식 (2025년 귀속분 파일 기준 — 신고 완료, 형식 참고용)
...
사용자가 "중간 정리본", "정리본 보여줘", "지금까지 정리해줘" 류의 요청을 하면
연말 워크플로우 전체가 아니라 **현재 장부 기준의 DRAFT 엑셀**을 생성해 전달한다.

1. 파일명: `2026_Tax_Summary_DRAFT_<오늘날짜>.xlsx`
2. 데이터 소스: `tax-data/2026-realized-gains.csv` (+ 필요 시 장부/이월 lot CSV 재계산)
3. Summary 시트의 환율은 미확정이면 `미확정 (TBD)`로 표기하고 USD 컬럼은 비워 둔다.
   임의 환율을 적용하지 않는다.
4. 배당·이자 시트는 데이터가 없으면 "미수집 — 연말 반영 예정" 안내 행만 넣는다.
5. SendUserFile로 전달하며 레포에는 커밋하지 않는다.
